...
const Camera: React.FC<CameraProps> = ({ fov, aspect, near, far }) => {
  ...
  useEffect(() => {
    if (ref.current && scene) {
      ...
      const direction = ref.current.position.clone().sub(center).normalize().multiplyScalar(distance);
  ...
  return <PerspectiveCamera ref={ref} fov={fov} aspect={aspect} near={near} far={far} />;
};
...
const MugScene: React.FC<MugSceneProps> = ({ svgDataURL }) => {
  return (
    <Canvas>
      <Camera fov={50} aspect={window.innerWidth / window.innerHeight} near={0.1} far={100} />
      <Lights />
      <OrbitControls />
